...
figure = plt.figure(figsize=(13, 5), dpi=120)
...
plot_canvas.get_tk_widget().grid(column=0, row=0, columnspan=8, padx=20, pady=10)
...
tk.Label(frame, textvariable=laeq_str, font=('Arial', 15), fg='black').grid(column=5, row=1, pady=10)

lamax_str = StringVar()
lamax_str.set('LAmax (dBA): 0')
tk.Label(frame, textvariable=lamax_str, font=('Arial', 15), fg='black').grid(column=5, row=2, pady=10)

cough_str = StringVar()
cough_str.set('Counted Cough : 0')
tk.Label(frame, textvariable=cough_str, font=('Arial', 15), fg='black').grid(column=4, row=2, pady=10)
...
def open_audio_file():
    ...
    if filename != '':
        ...
        stream, sr = load_audio(filename, block_len=55)
...
def draw_spl_data(spl: NDArray, sr:int):
...
def get_ticks(times: NDArray, interval: float = 1.0):
    start_t = audio_info['frame_start_t']
...
def get_times(spl: NDArray, sr: int, start: float):
